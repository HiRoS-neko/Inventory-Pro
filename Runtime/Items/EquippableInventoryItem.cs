...
namespace Devdog.InventoryPro
{
    ...
    /// </summary>
    public class EquippableInventoryItem : InventoryItemBase
    {
        [SerializeField]
        [Required]
        ...
        public Vector3 equipmentPosition => _equipmentPosition;

        public Quaternion equipmentRotation => _equipmentRotation;

        /// <summary>
        ///     Is the item currently equipped or not?
        /// </summary>
        public bool isEquipped => equippedToCollection != null;
        ...
        {
            equippedToCollection = equipSlot.characterCollection;
            Assert.IsNotNull(equippedToCollection,
                "ICharacterCollection's player reference not set! Forgot to assign to player?");

            foreach (var stat in
                     stats)
                stat.actionEffect = StatDecorator.ActionEffect.Add; // Force add effect for equippable stats.

            equipSlot.characterCollection.character.stats.SetAll(stats);
        ...
        public override GameObject Drop()
        {
            ...
            if (isEquipped)
            {
                var unEquipped = equippedToCollection.UnEquipItem(this, true);
                if (unEquipped == false) return null;
            }
        ...
        public override int Use()
        {
            ...
            if (isEquipped)
            {
                ...
                if (unequipped)
                {
                    ...
                    if (prevCollection != null) prevCollection.NotifyItemUsed(this, ID, prevIndex, currentStackSize);
        ...
        protected virtual CharacterUI GetBestEquipToCollection()
        {
            ...
            foreach (var col in InventoryManager.GetEquipToCollections())
            {
                ...
                if (best != null && (best.slot.item == null || last == null)) last = col;
            }
        ...
        ///     This is validated after CanSetItem, so the item can be rejected before it gets here, if it doesn't match
        ///     onlyAllowTypes.
        /// </summary>
        ...
        {
            if (addToInventory) return InventoryManager.CanAddItem(this);
