...
namespace Devdog.InventoryPro
{
    ...
    public class ItemDropHandler : ItemDropHandlerBase
    {
        ...
        /// </summary>
        public Vector3 dropOffsetVector = new(0.0f, 0.0f, 0.0f);
        ...
        /// </summary>
        public bool dropItemRaycastToGround;
        ...
        {
            var dropObj = item.gameObject;
            if (item.overrideDropObjectPrefab != null)
            {
                // Drop override drop object
                dropObj = Instantiate(item.overrideDropObjectPrefab);
            ...
            else if (item.rarity != null && item.rarity.dropObject != null)
            {
                ...
                dropObj = Instantiate(item.rarity.dropObject);
                var holder = dropObj.GetOrAddComponent<ItemTrigger>();
                holder.itemPrefab = item;
            }

            dropObj.GetOrAddComponent<ItemTrigger>();
            if (dropObj.GetComponent<ITriggerInputHandler>() == null) dropObj.AddComponent<ItemTriggerInputHandler>();
            return dropObj;
        }
        ...
        protected virtual Vector3 CalculateDropPosition(InventoryItemBase item, Vector3 initialLocation,
            Quaternion initialRotation)
        {
            if (dropAtMousePosition)
            {
                ...
                RaycastHit hit;
                if (Physics.Raycast(ray, out hit, maxDropDistance, layersWhenDropping)) return hit.point;
            }

            initialLocation += initialRotation * dropOffsetVector;

            var droppableFromDistanceUp = 10.0f;
            if (dropItemRaycastToGround)
            {
                ...
                {
                    var dist = Vector3.Distance(aboveHit.transform.position, initialLocation);
                    droppableFromDistanceUp =
                        Mathf.Clamp(dist - 0.1f, 0.1f, 10.0f); // Needs to be at least a little above the ground
                }

                RaycastHit hit;
                if (Physics.Raycast(initialLocation + Vector3.up * droppableFromDistanceUp, Vector3.down, out hit,
                        25.0f))
                    initialLocation =
                        hit.point + Vector3.up * 0.1f; // + a little offset to avoid it falling through the ground
            }
