...
{
    /// <summary>
    ///     The base item of all the inventory items, contains some default behaviour for items, which can (almost) all be
    ///     overriden.
    /// </summary>
    //    [DisallowMultipleComponent]
    public class InventoryItemBase : MonoBehaviour, IEquatable<InventoryItemBase>
    {
        ...
        public virtual LinkedList<ItemInfoRow[]> GetInfo()
        {
            ...
            {
                new("Weight", weight.ToString()),
                new("Required level", requiredLevel.ToString()),
                new("Category", category.name)
            });

            var extra = new List<ItemInfoRow>(3);
            if (sellPrice != null) extra.Add(new ItemInfoRow("Sell price", sellPrice.ToString()));
            if (buyPrice != null) extra.Add(new ItemInfoRow("Buy price", buyPrice.ToString()));

            if (isDroppable == false || isSellable == false || isStorable == false)
                extra.Add(new ItemInfoRow(
                    (!isDroppable ? "Not droppable" : "") + (!isSellable ? ", Not sellable" : "") +
                    (!isStorable ? ", Not storable" : ""), Color.yellow));

            if (extra.Count > 0) list.AddLast(extra.ToArray());
            ...
            foreach (var property in stats)
            {
                ...
                if (prop.showInUI)
                {
                    if (property.isFactor && property.isSingleValue)
                        extraProperties.Add(new ItemInfoRow(prop.statName, (property.floatValue - 1.0f) * 100 + "%",
        ...
        public virtual IList<ItemUsability> GetUsabilities()
        {
            ...
            if (itemCollection.canUseFromCollection)
                l.Add(new ItemUsability("Use", item => { itemCollection[index].TriggerUse(); }));

            if (currentStackSize > 1 && itemCollection.canPutItemsInCollection)
                l.Add(new ItemUsability("Unstack", item => { itemCollection[index].TriggerUnstack(itemCollection); }));

            if (isDroppable && itemCollection.canDropFromCollection)
                l.Add(new ItemUsability("Drop", item => { itemCollection[index].TriggerDrop(false); }));
        ...
        public virtual bool PickupItem()
        {
            ...
            var pickedUp = InventoryManager.AddItem(this);
            if (pickedUp)
                NotifyItemPickedUp();
        ...
        /// <param name="alsoNotifyCollection">
        ///     If the collection of the item didn't change in the process it's safe to notify the
        ///     collection.
        /// </param>
        ...
        public virtual bool CanUse()
        {
            ...
            if (VerifyCustomUseConditionals() == false) return false;

            foreach (var prop in stats)
                if (prop.actionEffect == StatDecorator.ActionEffect.Decrease)
                    if (prop.actionEffect == StatDecorator.ActionEffect.Decrease &&
                        prop.CanDoDecrease(PlayerManager.instance.currentPlayer.InventoryPlayer()) == false)
                    {
                        InventoryManager.instance.sceneLangDatabase.itemCannotBeUsedToLowStat.Show(name, description,
                            prop.stat.statName);
                        return false;
                    }
            ...
            foreach (var prop in usageRequirement)
                ...
                {
                    InventoryManager.instance.sceneLangDatabase.itemCannotBeUsedStatNotValid.Show(name, description,
                        prop.stat.statName);
                    return false;
                }
        ...
        /// </summary>
        /// <returns>
        ///     Returns -1 if in timeout / cooldown, returns -2 if item use failed, 0 is 0 items were used, 1 if 1 item was
        ///     used, 2 if 2...
        /// </returns>
        public virtual int Use()
        {
            if (itemCollection != null)
            {
                ...
                var overrideBehaviour = itemCollection.OverrideUseMethod(this);
                if (overrideBehaviour) return -2;
            }
            ...
            if (isInCooldown)
            {
                InventoryManager.langDatabase.itemIsInCooldown.Show(name, description,
                    lastUsageTime + cooldownTime - Time.timeSinceLevelLoad, cooldownTime);
                return -1;
            }
        ...
        /// <summary>
        ///     Use the cooldown from the category? If true the global cooldown will be used, if false a unique cooldown can be
        ///     set.
        /// </summary>
        [SerializeField]
        ...
        {
            get => _useCategoryCooldown;
            set => _useCategoryCooldown = value;
        }
        ...
        {
            get => (uint)Mathf.Max(1, _layoutSizeCols);
        ...
        {
            get => (uint)Mathf.Max(1, _layoutSizeRows);
            set => _layoutSizeRows = value;
        }

        public uint layoutSize => layoutSizeRows * layoutSizeCols;
        ...
        /// <summary>
        ///     The minimal required level to use this item. This is not used directly by Inventory Pro but can be used by 3rd
        ///     party assets / custom code.
        /// </summary>
        ...
        /// </summary>
        [field: NonSerialized]
        public virtual uint currentStackSize { get; set; } = 1;
        ...
        public virtual float cooldownTime
        {
            get
            {
                if (useCategoryCooldown) return category.cooldownTime;

                return _cooldownTime;
            }
            protected set => _cooldownTime = value;
        }

        private static readonly Dictionary<uint, float> _lastUsageTimeLookup = new();
        ...
        public virtual float lastUsageTime
        {
            get
            {
                if (useCategoryCooldown) return category.lastUsageTime;
        ...
        public bool isInCooldown => Time.timeSinceLevelLoad - lastUsageTime < cooldownTime && lastUsageTime > 0f;
        ...
        /// </summary>
        public float cooldownFactor => (Time.timeSinceLevelLoad - lastUsageTime) / cooldownTime;

        protected virtual ItemDropHandlerBase dropHandler => InventorySettingsManager.instance.settings.itemDropHandler;
