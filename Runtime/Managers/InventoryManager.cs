...
namespace Devdog.InventoryPro
{
    ...
    public class InventoryManager : ManagerBase<InventoryManager>
    {
        ...
        public Camera uiCamera => _uiCamera ?? Camera.main;
        ...
        public static InventoryDatabaseLookup<LangDatabase> langDatabaseLookup
        {
            get
            {
                if (_langDatabaseLookup == null)
                    _langDatabaseLookup =
                        new InventoryDatabaseLookup<LangDatabase>(instance != null ? instance.sceneLangDatabase : null,
                            CurrentLangDBPathKey);
        ...
        {
            get => langDatabaseLookup.GetDatabase();
        ...
        public static string CurrentLangDBPathKey =>
            CurrentDBPrefixName + InventoryPro.ProductName + "_CurrentLangDatabasePath";
        ...
        ///     When an item is picked up the item will be moved to the inventory. You can create multiple Inventories and limit
        ///     types per inventory.
        /// </summary>
        private static readonly List<ItemCollectionPriority<ItemCollectionBase>> _lootToCollections = new();

        private static readonly List<ItemCollectionPriority<CharacterUI>> _equipToCollections = new(4);
        private static readonly List<ItemCollectionBase> _bankCollections = new(4);
        ...
        protected override void Start()
        {
            ...
            SceneManager.sceneLoaded += (scene, loadMode) => { LevelLoaded(scene.buildIndex); };
#endif
        }
        ...
        protected virtual void LevelLoaded(int level)
        {
            ...
            foreach (var category in ItemManager.database.categories) category.lastUsageTime = 0.0f;
        }
        ...
        {
            if (_counter == null) _counter = new ItemCollectionBaseAddCounter();

            _counter.LoadFrom(_lootToCollections.ToArray());
        }

        protected virtual ItemCollectionPriority<ItemCollectionBase> GetBestLootCollectionForItem(
            InventoryItemBase item, bool rebuildCounter)
        {
            if (rebuildCounter) RebuildCounter();
        ...
        public static List<InventoryItemBase> FindAllLike(InventoryItemBase item, bool checkBank)
        {
            ...
            foreach (var col in _lootToCollections) list.AddRange(col.collection.FindAllLike(item));
        ...
        {
            var list = new List<InventoryItemBase>(8);
            foreach (var col in _lootToCollections) list.AddRange(col.collection.FindAll(itemID));
        ...
        /// <returns></returns>
        public static bool AddItem(InventoryItemBase item, ICollection<InventoryItemBase> storedItems = null,
            bool repaint = true)
        {
            Assert.IsNotNull(item, "Given item is null - Can't add.");
            Assert.IsTrue(_lootToCollections.Count > 0,
                "Can't add item, no collectionso attached to the current player");

            var currency = item as CurrencyInventoryItem;
            if (currency != null)
            {
                var added = AddCurrency(currency.currency.ID, currency.amount);
                if (added) Destroy(currency.gameObject);
            ...
            {
                var added = bestCollection.collection.AddItem(i);
                Assert.IsTrue(added, "Item wasn't added even though check passed!");

                if (storedItems != null) storedItems.Add(item);
            }
        ...
        /// <returns></returns>
        public static bool AddItems(InventoryItemBase[] items, ICollection<InventoryItemBase> storedItems = null,
            bool repaint = true)
        {
            items = items.OrderByDescending(o => o.layoutSize)
                .ToArray(); // Order from large to small to make sure large items can always be placed.
        ...
        /// <param name="storedItems">The items that were stored, item might be broken up into stacks</param>
        /// <returns></returns>
        public static bool AddItemAndRemove(InventoryItemBase item, ICollection<InventoryItemBase> storedItems = null,
            bool repaint = true)
        {
            ...
            if (best != null) return best.collection.AddItemAndRemove(item, storedItems, repaint);
        ...
        {
            RebuildCounter();
            var removeSucceeded = _counter.TryRemoveItems(itemsToRemoveFirst);
            if (removeSucceeded == false) return false;
            ...
            {
                var removed = RemoveItem(item.item.ID, item.amount, false);
                Assert.IsTrue(removed == item.amount, "Couldn't remove item even though check passed!");
            }

            var added = AddItems(InventoryItemUtility.RowsToItems(itemsToAdd, true));
            Assert.IsTrue(added);
        ...
        /// </summary>
        /// <returns></returns>
        public static bool CanRemoveItemsThenAdd(InventoryItemBase[] itemsToAdd,
            params ItemAmountRow[] itemsToRemoveFirst)
        {
        ...
        {
            Assert.IsTrue(GetItemCount(itemID, checkBank) >= amount,
                "Tried to remove more items than available, check with FindAll().Count first.");
        ...
        public static float GetCurrencyCount(uint currencyID, bool checkBank)
        {
            ...
            foreach (var col in _lootToCollections)
            {
                ...
                if (c != null) totalAmount += c.amount;
            }
        ...
        /// <param name="allowCurrencyConversion">
        ///     Allow converting a higher currency down to this currency? For example convert
        ///     gold to silver.
        /// </param>
        /// <param name="checkBank"></param>
        /// <returns></returns>
        public static bool CanRemoveCurrency(CurrencyDecorator currencyDecorator, bool allowCurrencyConversion,
            bool checkBank)
        {
            if (currencyDecorator.amount <= 0f) return true;

            Assert.IsNotNull(currencyDecorator.currency,
                "Currency decorator is empty! The currency cannot be removed!");
            return CanRemoveCurrency(currencyDecorator.currency.ID, currencyDecorator.amount, allowCurrencyConversion,
                checkBank);
        }
            ...
            bool checkBank)
        {
            ...
            var totalAmount = CanRemoveCurrencyCountInventories(currencyID, allowCurrencyConversion);
            if (checkBank) totalAmount += CanRemoveCurrencyCountBanks(currencyID, allowCurrencyConversion);
        ...
        public static bool AddCurrency(uint currencyID, float amount)
        {
            ...
            foreach (var col in _lootToCollections)
            {
                ...
                var canAdd = col.collection.CanAddCurrencyCount(currencyID);
                if (canAdd >= toAdd)
                {
            ...
            Assert.IsTrue(false,
                "Couldn't add currency even though check passed! Please report this error + stack trace");
            return false;
        }

        public static bool RemoveCurrency(CurrencyDecorator decorator, float amountMultipier = 1.0f)
        {
            if (decorator.amount <= 0f) return true;
        ...
        {
            if (CanRemoveCurrency(currencyID, amount, true) == false) return false;
            ...
            foreach (var col in _lootToCollections)
            {
                var canRemove = col.collection.CanRemoveCurrencyCount(currencyID);
            ...
            Assert.IsTrue(false,
                "Couldn't remove currency even though check passed! Please report this error + stack trace");
            return false;
        }
