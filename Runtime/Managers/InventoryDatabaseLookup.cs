...
{
    public class InventoryDatabaseLookup<T> where T : ScriptableObject
    {
        private T _database;
        ...
        public bool hasSelectedDatabase
        {
            get
            {
                if (defaultDatabase != null) return true;
        ...
        public T GetDatabase()
        {
            ...
            if (defaultDatabase != null) return defaultDatabase;
            ...
            {
                var path = EditorPrefs.GetString(key);
                var set = SetDatabase(AssetDatabase.LoadAssetAtPath<T>(path));
                if (set == false)
                {
                    // Path is no longer valid
                    EditorPrefs.DeleteKey(key);
                    throw new DatabaseNotFoundException(typeof(T).Name +
                                                        " could not be found at previously set path. Re-select a database.");
                }
            }

            // Still null after selecting?
            if (_database == null) ManuallySelectDatabase();
        ...
        public bool SetDatabase(T database)
        {
            ...
            if (_database != null)
            {
                if (AssetDatabase.GetAssetPath(before) != AssetDatabase.GetAssetPath(_database) && before != null)
                    EditorUtility.DisplayDialog("Switched database",
                        "Switched to a new database (" + _database.name + ")", "Ok");

                var path = AssetDatabase.GetAssetPath(_database);
...
//            }
            ...
            var select = EditorUtility.DisplayDialog("Select an " + typeof(T).Name,
                "No " + typeof(T).Name + " could be found, please select one manually.", "Select database",
                "Create database");
            if (select)
            {
                var absolutePath = EditorUtility.OpenFilePanel(
                    "No " + typeof(T).Name + " could be found, please select it manually.",
                    "Assets/InventorySystem/Demos/Assets/Databases/", "asset");
                var set = SetDatabase(
                    AssetDatabase.LoadAssetAtPath<T>("Assets" + absolutePath.Replace(Application.dataPath, "")));
