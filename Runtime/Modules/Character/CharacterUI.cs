...
namespace Devdog.InventoryPro
{
    ...
    public class CharacterUI : ItemCollectionBase, ICollectionPriority, IInventoryDragAccepter, ICharacterCollection
    {
        [Range(0, 100)]
        [SerializeField]
        ...
        public override uint initialCollectionSize => (uint)equippableSlots.Length;

        protected override void Awake()
        {
            Assert.IsTrue(manuallyDefineCollection,
                "Character collections should manually be defined in the editor. If you want to generate an editor equipment system at run-time inherit from this character collection or write your own.");
            base.Awake();
        ...
        public EquippableSlot[] GetEquippableSlots(EquippableInventoryItem item)
        {
            if (item.equipmentType == null)
            {
                Debug.LogWarning(
                    "The item " + item.name +
                    " you're trying to equip has no equipment type set. Item cannot be equipped and will be ingored.",
                    item.gameObject);
                return new EquippableSlot[0];
            }

            if (equippableSlots.Length == 0)
            {
            ...
            foreach (var field in equippableSlots)
            {
                ...
                if (field.gameObject.activeSelf == false || field.enabled == false) continue;
                ...
                foreach (var type in field.equipmentTypes)
                    if (item.equipmentType == type)
                    {
                        ...
                        foreach (var prop in item.usageRequirement)
                        {
                            ...
                            if (canAdd == false) break;
                        }

                        if (canAdd) equipSlots.Add(field);
                    }
            }
        ...
        public bool EquipItem(EquippableSlot equipSlot, EquippableInventoryItem item)
        {
            ...
            var handled = HandleLocks(equipSlot, item);
            ...
            {
                var unEquipped = UnEquipItem((EquippableInventoryItem)items[equipSlot.index].item, true);
                if (unEquipped == false) return false;
            }

            // EquippedItem the item -> Will swap as merge is not possible
            var canSet = CanSetItem(equipSlot.index, item);
            if (canSet)
            {
                var set = SetItem(equipSlot.index, item);
        ...
        public bool UnEquipItem(EquippableInventoryItem item, bool addToInventory)
        {
            ...
            Assert.IsTrue(item.itemCollection == this || useReferences,
                "Trying to un-equip an item that wasn't equipped in this collection!");

            if (item.CanUnEquip(addToInventory) == false) return false;

            if (useReferences == false)
            {
                if (addToInventory)
                {
                    var added = InventoryManager.AddItemAndRemove(item);
                    Assert.IsTrue(added);
                    return added;
                }
        ...
        public virtual void UpdateEquippableSlots()
        {
            ...
            for (var i = 0; i < items.Length; i++)
            {
                ...
                if (c != null)
                {
                    ...
                    Assert.IsNotNull(equippableSlots[i],
                        "CharacterUI manually defined collection contains gameObject without InventoryEquippableField component.");

                    if (equippableSlots[i].equipmentTypes.Any(o => o == null))
                        DevdogLogger.LogError("CharacterUI's equipTypes contains null (empty) field.",
                            equippableSlots[i].gameObject);
                }
            }

            if (equippableSlots.Any(o => o == null))
                DevdogLogger.LogError(
                    "This characterUI has an empty reference in the equip slot fields (EquippableFields has empty row)",
                    gameObject);
        }
        ...
        public override bool CanSetItem(uint slot, InventoryItemBase item)
        {
            ...
            if (equippable.VerifyCustomUseConditionals() == false) return false;

            if (equippable.CanEquip(this) == false) return false;
        ...
        protected virtual bool HandleLocks(EquippableSlot equipSlot, EquippableInventoryItem equippable)
        {
            ...
            foreach (var field in equippableSlots)
            {
                ...
                if (item != null)
                {
                    ...
                    if (eq.equipmentType == blockType && field.index != equipSlot.index)
                    {
                        ...
                        var canAdd = InventoryManager.CanAddItem(eq);
            ...
            {
                var added = InventoryManager.AddItemAndRemove(items[i].item);
        ...
        public override bool SetItem(uint slot, InventoryItemBase item)
        {
            ...
            if (equippable != null)
            {
                var handled = HandleLocks(equippableSlots[slot], equippable);
                if (handled == false) return false;
            }
        ...
        public override void NotifyItemAdded(IEnumerable<InventoryItemBase> items, uint amount, bool cameFromCollection)
        {
            ...
            {
                Assert.IsTrue(item is EquippableInventoryItem,
                    "Non equippable item was added to character collection. This is not allowed.");
                ...
                if (i.equipVisually) _character.equipmentHandler.EquipItemVisually(i, slot);
            }
        }
        ...
        public override void NotifyItemRemoved(InventoryItemBase item, uint itemID, uint slot, uint amount)
        {
            ...
            foreach (var wrapper in items)
                if (wrapper.item != null)
                {
                    ...
                    var canEquip = equip.CanEquip(this);
                    if (canEquip == false && equip.isEquipped) UnEquipItem(equip, true);
                }
        ...
        protected override bool SwapSlots(uint fromSlot, ItemCollectionBase toCollection, uint toSlot,
            bool repaint = true, bool fireEvents = true)
        {
            var swapped = base.SwapSlots(fromSlot, toCollection, toSlot, repaint, fireEvents);
            if (swapped)
            {
                if (this != toCollection) return true;
