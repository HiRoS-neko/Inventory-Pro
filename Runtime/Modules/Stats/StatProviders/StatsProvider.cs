...
namespace Devdog.InventoryPro
{
    public class StatsProvider : IStatsProvider
    {
        public Dictionary<string, List<IStat>> Prepare()
        {
            ...
            {
                if (stat == null || stat.enabled == false) continue;

                if (appendTo.ContainsKey(stat.category) == false) appendTo.Add(stat.category, new List<IStat>());

                // Check if it's already in the list
                if (appendTo[stat.category].FirstOrDefault(o => o.definition.Equals(stat)) != null) continue;
