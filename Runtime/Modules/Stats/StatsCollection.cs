...
//        }
        ...
        {
            if (ContainsCategory(category) == false) return null;
        ...
        {
            if (ContainsCategory(category) == false) _stats.Add(category, new List<IStat>());

            if (_stats[category].Any(o => o == stat))
                Debug.LogWarning("Stat with same name already exists in this characterUI - " +
                                 stat.definition.statName +
                                 " will still be added. Use GetStat() == null to verify if it exists first.");
...
//            stat.OnExperienceChanged += NotifyStatExperienceChanged;
        ...
        {
            if (_stats.ContainsKey(category) == false) return false;
...
//                stat.OnExperienceChanged -= NotifyStatExperienceChanged;
        ...
        public void ChangeAll(StatDecorator[] stats, float multiplier = 1f, bool fireEvents = true)
        {
            foreach (var stat in stats)
            {
                ...
                if (s != null) s.ChangeCurrentValueRaw(stat.floatValue * multiplier, fireEvents);
            }
        }

        public void SetAll(StatDecorator[] stats, float multiplier = 1f, bool fireEvents = true)
        {
            foreach (var stat in stats) Set(stat, multiplier, fireEvents);
        }
        ...
        {
            if (OnStatValueChanged != null) OnStatValueChanged(stat);
        }

        protected void NotifyStatLevelChanged(IStat stat)
        {
            if (OnStatLevelChanged != null) OnStatLevelChanged(stat);
        }
    }
}