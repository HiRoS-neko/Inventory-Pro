...
namespace Devdog.InventoryPro
{
    public abstract class ItemEquipmentHandlerBase : ScriptableObject
    {
        ...
        [SerializeField]
        protected bool forceReplaceAllBones;
...
//        protected bool copySphereColliders;
        ...
        protected EquippableInventoryItem CreateDefaultCopy(EquippableInventoryItem item)
        {
            ...
            Destroy(copy.GetComponent<ITriggerInputHandler>() as Component);
            ...
            InventoryUtility.SetLayerRecursive(copy.gameObject,
                InventorySettingsManager.instance.settings.equipmentLayer);

            if (resizeItemsToScale1) copy.transform.localScale = Vector3.one;
        ...
        {
            if (binder.equipTransform == null) return;
                ...
                if (itemSkinned != null)
                {
                    ...
                    if (playerSkinned != null)
                    {
                        ...
                        if (forceReplaceAllBones)
                        {
                            ...
                            DevdogLogger.LogVerbose(
                                "Force copied " + itemSkinned.bones.Length + " bones to new skinned mesh", itemSkinned);
                        }
                        else
                        {
                    ...
                    else
                    {
                        DevdogLogger.LogWarning(
                            "Tried to equip skinned item to player, but no skinned mesh renderer found in equip position's parent.",
                            binder.equipTransform);
                    }
                }
            }
        }
        ...
        protected void ReplaceBonesOnTarget(SkinnedMeshRenderer source, SkinnedMeshRenderer writeTo)
        {
            ...
            foreach (var t in writeTo.bones)
            {
                ...
                if (bone != null) newBones.Add(bone);
            }

            writeTo.bones = newBones.ToArray();
            DevdogLogger.LogVerbose(
                "Merged " + newBones.Count + "/" + boneCountBefore + " bones from " + writeTo.gameObject.name + " to " +
                source.gameObject.name + " (source has " + source.bones.Length + " bones)", source);
        }
        ...
        public abstract EquippableInventoryItem Equip(EquippableInventoryItem item, CharacterEquipmentTypeBinder binder,
            bool createCopy);
