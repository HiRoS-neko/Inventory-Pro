...
namespace Devdog.InventoryPro
{
    ...
    public class CharacterEquipmentHandler : CharacterEquipmentHandlerBase
    {
        /// <summary>
        ...
        public override void SwapItems(ICharacterCollection collection, uint fromSlot, uint toSlot)
        {
            ...
            if (fromItem != null) UnEquipItemVisually(fromItem);

            EquipItemVisually(toItem, characterCollection.equippableSlots[toSlot]);

            if (fromItem != null) EquipItemVisually(fromItem, characterCollection.equippableSlots[fromSlot]);
        }
        ...
        {
            if (item.equipVisually == false) return;
        ...
        public override EquippableSlot FindEquippableSlotForItem(EquippableInventoryItem equippable)
        {
            ...
            return characterCollection.equippableSlots.FirstOrDefault(o =>
                o.equipmentTypes.Contains(equippable.equipmentType));
        }

        private EquipmentType GetEquippableTypeFromItem(CharacterEquipmentTypeBinder binder,
            EquippableInventoryItem item)
        {
        ...
        {
            if (item.visuallyEquippedToBinder != null) UnEquipItemVisually(item, item.visuallyEquippedToBinder);
        }
