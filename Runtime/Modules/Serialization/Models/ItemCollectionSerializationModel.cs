...
namespace Devdog.InventoryPro
{
    ...
    public class ItemCollectionSerializationModel
    {
        ...
        {
            currencies = collection.currenciesGroup.lookups.Select(o => new CurrencyDecoratorSerializationModel(o))
                .ToArray();
            ...
            for (var i = 0; i < collection.items.Length; i++)
            {
                ...
                if (item.item != null)
                {
                    var classes = ReflectionUtility.GetAllClassesWithAttribute(typeof(SerializationModelAttribute));
                    var serializationModel = typeof(InventoryItemSerializationModel);

                    foreach (var c in classes)
                    {
                        var attrib =
                            (SerializationModelAttribute)c
                                .GetCustomAttributes(typeof(SerializationModelAttribute), true).First();
                        if (c == item.item.GetType())
                        {
                            DevdogLogger.LogVerbose("Using custom serialization model for " + item.item.GetType().Name +
                                                    " - " + attrib.type.Name);
        ...
        public virtual void ToCollection(ItemCollectionBase collection)
        {
            ...
            foreach (var c in deserializedCurrencies) collection.currenciesGroup.AddCurrency(c);
