...
namespace Devdog.InventoryPro
{
    public abstract class CollectionSaverLoaderBase : SaverLoaderBase
    {
        ...
        protected ItemCollectionBase collection
        {
            get
            {
                if (_collection == null) _collection = GetComponent<ItemCollectionBase>();
        ...
        public override string saveName =>
            SaveNamePrefix + "Collection_" + collection.collectionName.ToLower().Replace(" ", "_");
        ...
        public override void Save()
        {
            try
            {
                ...
                SaveItems(serializedCollection,
                    saved => { DevdogLogger.LogVerbose("Saved collection " + collection.collectionName); });
            }
        ...
        public override void Load()
        {
            try
            {
                LoadItems(data =>
                {
                    ...
                    var character = collection.GetComponent<ICharacterCollection>();
                    ...
                    if (character != null)
                        ...
                        foreach (var slot in collection.items)
                        {
                            ...
                            if (equippedItem != null && equippedItem.equipVisually)
                                character.character.equipmentHandler.EquipItemVisually(equippedItem,
                                    character.equippableSlots[equippedItem.index]);
                        }
                });
            }
            catch (SerializedObjectNotFoundException e)
