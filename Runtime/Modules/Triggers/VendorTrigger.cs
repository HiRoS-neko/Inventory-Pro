...
namespace Devdog.InventoryPro
{
    ...
    public class VendorTrigger : MonoBehaviour, IInventoryItemContainer, ITriggerCallbacks, ITriggerWindowContainer
    {
        [Header("Vendor")]
...
//        [Header("Items")]
        ...
        /// <summary>
        ///     All prices are multiplied with this value. If you want to make items 10% more expensive in a certain are, or on a
        ///     certain vendor, use this.
        /// </summary>
        [Range(0.0f, 10.0f)]
        ...
        /// <summary>
        ///     All sell prices are multiplied with this value. If you want to make items 10% more expensive in a certain are, or
        ///     on a certain vendor, use this.
        /// </summary>
        [Range(0.0f, 10.0f)]
        ...
        /// <summary>
        ///     When true the items will be stored in the buy back collection, when false the items will be added to the regular
        ///     collection of the vendor.
        /// </summary>
        [Tooltip(
        ...
        /// </summary>
        public bool buyBackIsShared;

        /// <summary>
        ///     The category this vendor belongs to, used for sharing the buyback.
        ///     Shared buyback is shared based on the vendor categeory, all vendors with the same category will have the same buy
        ///     back items.
        /// </summary>
        [Tooltip(
        ...
        public new string name => uniqueName;
        ...
        protected VendorUI vendorUI => InventoryManager.instance.vendor;
        ...
        protected virtual void Start()
        {
            ...
            buyBackDataStructure = new VendorBuyBackDataStructure<InventoryItemBase>((int)maxBuyBackItemSlotsCount,
                buyBackIsShared, vendorCategory, (int)maxBuyBackItemSlotsCount);
        ...
        public UIWindow window => vendorUI.window;
        ...
        public bool OnTriggerUsed(Player player)
        {
            // Set items
            vendorUI.Clear();
            if (items.Length > vendorUI.items.Length) vendorUI.Resize((uint)items.Length);

            for (uint i = 0; i < items.Length; i++) vendorUI.AddItem(items[i], null, true, false);
        ...
        public bool OnTriggerUnUsed(Player player)
        {
            ...
            if (before == this) vendorUI.window.Hide();
        ...
        {
            var max = InventoryManager.GetItemCount(item.ID, false);
        ...
        {
            var totalCost = GetBuyBackPrice(item, amount);
            ...
            {
                var totalCostString = item.sellPrice.ToString(amount * buyBackPriceFactor);
                var c = InventoryManager.GetCurrencyCount(item.sellPrice.currency.ID, false);

                InventoryManager.langDatabase.userNotEnoughGold.Show(item.name, item.description, amount,
                    totalCostString, c);
        ...
        {
            var totalCost = GetBuyPrice(item, amount);

            if (totalCost > 0f)
                if (InventoryManager.CanRemoveCurrency(item.buyPrice.currency.ID, totalCost, true) == false)
                {
                    var totalCostString = item.buyPrice.ToString(amount * buyPriceFactor);
        ...
        {
            var action = ItemBuySellDialogAction.Buying;
            var maxAmount = removeItemAfterPurchase ? vendorUI.GetItemCount(item.ID) : maxBuyItemCount;
            if (isBuyBack)
            {
        ...
        public virtual bool BuyItemFromVendorNow(InventoryItemBase item, uint amount)
        {
            ...
            if (buyPrice > 0f) InventoryManager.RemoveCurrency(item.buyPrice.currency.ID, buyPrice);
            c1.PickupItem();
            //            InventoryManager.AddItem(c1); // Will handle unstacking if the stack size goes out of bounds.

            if (removeItemAfterPurchase) vendorUI.RemoveItem(item.ID, amount);
        ...
        {
            var originalStackSize = item.currentStackSize;

            item.currentStackSize = amount;
            var can = InventoryManager.CanAddItem(item);
