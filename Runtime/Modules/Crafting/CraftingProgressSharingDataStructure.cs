...
namespace Devdog.InventoryPro
{
    public enum CraftingProgressShareSetting
    {
        ...
        /// <summary>
        ///     When enabled the progress will be stored per crafting category, allowing the user to craft 1 item at a time per
        ///     category.
        /// </summary>
...
//        private static List<T> _list;
        ...
        public CraftingProgressSharingDataStructure(CreateNewInstance createNewAction, T singleInstance,
            int reserve = 0)
        {
...
//            _list = new List<T>(reserve);
        ...
        {
            return setting == CraftingProgressShareSetting.Default ||
                   setting == CraftingProgressShareSetting.ForceSingleInstancePerCategory;
        }
        ...
        public CraftingProgressContainer Get(ICraftingActionValidator validator, int instanceID, int category,
            CraftingProgressShareSetting shareSetting)
        {
            if (IsShared(shareSetting))
            {
...
//            return Get(hash, category, shareSetting);
        ...
        public void Remove(int instanceID, int category, CraftingProgressShareSetting shareSetting)
        {
            if (IsShared(shareSetting))
            {
                if (_sharedDict.ContainsKey(category)) _sharedDict[category].RemoveAll(o => o.instanceID == instanceID);
            }
        ...
        {
            if (IsShared(shareSetting)) return _sharedDict[category].ToArray();
