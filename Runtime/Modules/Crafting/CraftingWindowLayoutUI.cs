...
namespace Devdog.InventoryPro
{
    ...
    public class CraftingWindowLayoutUI : CraftingWindowBase
    {
        ...
        public virtual void ValidateReferences(CraftingCategory category)
        {
            ...
            foreach (var wrapper in mainItemsCollection.items)
                ...
                {
                    var count = InventoryManager.GetItemCount(wrapper.item.ID, category.alsoScanBankForRequiredItems);
        ...
        {
            foreach (var wrapper in mainItemsCollection.items) wrapper.Repaint();
        }
        ...
        public override void SetCraftingCategory(CraftingCategory category)
        {
            ...
            if (category.cols * category.rows > removeItemsFromCollection.items.Length)
            {
                ...
                removeItemsFromCollection.AddSlots((uint)(category.cols * category.rows -
                                                          removeItemsFromCollection.items.Length));
            }
            else if (category.cols * category.rows < removeItemsFromCollection.items.Length)
            {
                ...
                removeItemsFromCollection.RemoveSlots((uint)(removeItemsFromCollection.items.Length -
        ...
        {
            var can = base.CanCraftBlueprint(player, craftInfo);
            if (can == false) return false;

            // No blueprint found
            if (GetBlueprintFromCurrentLayout(craftInfo.removeItemsFromCollection ?? mainItemsCollection,
                    craftInfo.category) == null) return false;
        ...
        public virtual CraftingBlueprint GetBlueprintFromCurrentLayout(ItemCollectionBase collection,
            CraftingCategory category)
        {
            ...
            var totalItemCountInLayout = 0; // Nr of items inside the UI wrappers.
            ...
            foreach (var a in b.blueprintLayouts)
                if (a.enabled)
                {
                    ...
                    var counter = 0; // Item index counter
                    var shouldHaveCount = 0; // Amount we should have..
                    var hasCount = 0; // How many slots in our layout
                                ...
                                if (collection.items[counter].item != null)
                                {
                                    ...
                                    if (hasItems.ContainsKey(c.item.ID) == false)
                                    {
                                        ...
                                        if (collection.useReferences)
                                            itemCount = InventoryManager.GetItemCount(c.item.ID,
                                                category.alsoScanBankForRequiredItems);
                                        else
