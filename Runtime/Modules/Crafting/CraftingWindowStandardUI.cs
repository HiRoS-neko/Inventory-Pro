...
namespace Devdog.InventoryPro
{
    ...
    public class CraftingWindowStandardUI : CraftingWindowBase
    {
        /// <summary>
        ...
        {
            var amount = GetCraftInputFieldAmount();
...
//            }
            ...
            foreach (var b in GetBlueprints(category))
            {
                ...
                if (firstButton == null) firstButton = blueprintObj.button;
                ...
                blueprintObj.button.onClick.AddListener(() =>
                {
                    ...
                    if (blueprintCraftPage != null && blueprintCraftPage.isVisible == false) blueprintCraftPage.Show();
                });
            }
        ...
        public override void SetCraftingBlueprint(CraftingBlueprint blueprint)
        {
            ...
            foreach (var item in blueprint.requiredItems)
            {
                ...
                item.item.currentStackSize = item.amount;
                ui.transform.SetParent(blueprintRequiredItemsContainer);
                InventoryUtility.ResetTransform(ui.transform);

                ui.item = item.item;
                if (InventoryManager.GetItemCount(item.item.ID, currentCategory.alsoScanBankForRequiredItems) >=
                    item.amount)
