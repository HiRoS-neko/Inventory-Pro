...
namespace Devdog.InventoryPro
{
    ...
    public abstract class CraftingWindowBase : MonoBehaviour, ICraftingActionValidator
    {
        ...
        [SerializeField]
        private bool _forceSingleInstance;
        ...
        [Tooltip(
            "All rewards will be stored in this collection. If the field is empty, the items will be added to the player's inventory auto.")]
        ...
        /// <summary>
        ///     The progress container contains all of the crafting progress. This reference can be shared between triggers,
        ///     categories, etc; However you like.
        /// </summary>
        [NonSerialized]
        ...
        public CraftingCategory startCraftingCategory => _startCraftingCategory;
        ...
        public virtual bool CanCraftBlueprint(InventoryPlayer player, CraftingProgressContainer.CraftInfo craftInfo)
        {
            ...
            if (player.characterUI != null)
                foreach (var propLookup in craftInfo.blueprint.usageRequirement)
                    ...
                    {
                        InventoryManager.langDatabase.craftingCannotStatNotValid.Show(craftInfo.blueprint.name,
                            craftInfo.blueprint.description, propLookup.stat.statName);
                        return false;
                    }

            if (CanRemoveRequiredItems(craftInfo) == false)
            {
                InventoryManager.langDatabase.craftingDontHaveRequiredItems.Show(
                    craftInfo.blueprint.requiredItems[0].item.name,
                    craftInfo.blueprint.requiredItems[0].item.description, craftInfo.blueprint.name);
                return false;
            }

            if (CanAddItemsRewardItems(craftInfo) == false)
            {
                InventoryManager.langDatabase.collectionFull.Show(craftInfo.blueprint.name,
                    craftInfo.blueprint.description, "Reward collection");
                return false;
            }
            ...
            {
                InventoryManager.langDatabase.userNotEnoughGold.Show(craftInfo.blueprint.name,
                    craftInfo.blueprint.description, craftInfo.craftAmount,
                    craftInfo.blueprint.craftingCost.ToString(craftInfo.craftAmount));
                return false;
            }
        ...
        public virtual void RemoveRequiredCraftItemsAndCurrency(CraftingProgressContainer.CraftInfo craftInfo)
        {
            ...
            if (craftInfo.removeItemsFromCollection != null &&
                craftInfo.removeItemsFromCollection.useReferences == false)
                ...
                foreach (var item in craftInfo.blueprint.requiredItems)
                    InventoryManager.RemoveItem(item.item.ID, item.amount,
                        craftInfo.category.alsoScanBankForRequiredItems);
        ...
        public virtual void GiveCraftReward(CraftingProgressContainer.CraftInfo craftInfo)
        {
            ...
            if (craftInfo.storeItemsInCollection != null)
            {
                ...
                var added = craftInfo.storeItemsInCollection.AddItems(itemsToAdd);

                craftInfo.storeItemsInCollection.canPutItemsInCollection = before;

                Assert.IsTrue(added,
                    "Couldn't add items even though check passed. Please report this error + stack trace.");
            }
            else
            {
                // Store crafted item
                var added = InventoryManager.AddItems(itemsToAdd);
                Assert.IsTrue(added,
                    "Couldn't add items even though check passed. Please report this error + stack trace.");
            }
        }
        ...
        protected virtual void InstanceOnPlayerChanged(Player oldPlayer, Player newPlayer)
        {
            ...
            foreach (var col in subscribedToCollection) col.OnCurrencyChanged += OnPlayerCurrencyChanged;
        }

        private void OnPlayerCurrencyChanged(float amountBefore, CurrencyDecorator decorator)
        {
            if (currentBlueprint != null) SetCraftingBlueprint(currentBlueprint);
        }
        ...
        {
            if (_cancelCraftingOnWindowClose) progressContainer.CancelActiveCraftAndClearQueue();
        }
        ...
        public virtual void CraftCurrentlySelectedBlueprint(int amount)
        {
            ...
            progressContainer.AddBlueprintToCraftingQueue(PlayerManager.instance.currentPlayer.InventoryPlayer(),
                currentCategory, currentBlueprint, amount, storeRewardItemsInCollection, removeItemsFromCollection);
        }
        ...
        public virtual void UseWithTrigger(CraftingCategory category, CraftingProgressContainer progressContainer,
            ICraftingActionValidator validator, ItemCollectionBase removeItemsFromCollection,
            ItemCollectionBase storeRewardItemsInCollection)
        {
            ...
            if (_forceSingleInstance)
            {
                this.progressContainer =
                    new CraftingProgressContainer(this, GetInstanceID(), GetComponent<AudioSource>());
            }
            else
            {
        ...
        {
            return InventoryManager.CanRemoveCurrency(craftInfo.blueprint.craftingCost, true,
                craftInfo.category.alsoScanBankForRequiredItems);
        }
        ...
        protected virtual bool CanAddItemsRewardItems(CraftingProgressContainer.CraftInfo craftInfo)
        {
            if (craftInfo.storeItemsInCollection != null)
            {
                ...
                for (var i = 0; i < craftInfo.craftAmount; i++) l.AddRange(craftInfo.blueprint.resultItems);

                if (craftInfo.storeItemsInCollection.CanAddItems(l.ToArray()) == false) return false;

                var stored = craftInfo.storeItemsInCollection.CanAddItems(l.ToArray());
            ...
            if (InventoryManager.IsInventoryCollection(removeItemsFromCollection))
                return InventoryManager.CanRemoveItemsThenAdd(craftInfo.blueprint.resultItems,
                    craftInfo.blueprint.requiredItems);
        ...
        protected virtual bool CanRemoveRequiredItems(CraftingProgressContainer.CraftInfo craftInfo)
        {
                ...
                craftInfo.removeItemsFromCollection.useReferences == false)
            {
                foreach (var item in craftInfo.blueprint.requiredItems)
                {
                    var count = craftInfo.removeItemsFromCollection.GetItemCount(item.item.ID);
                    if (count < item.amount * craftInfo.craftAmount) return false;
                }
            ...
            foreach (var item in craftInfo.blueprint.requiredItems)
            {
                var count = InventoryManager.GetItemCount(item.item.ID,
                    craftInfo.category.alsoScanBankForRequiredItems);
                if (count < item.amount * craftInfo.craftAmount) return false;
            }
        ...
        protected virtual void SetBlueprintResults(CraftingBlueprint blueprint)
        {
            if (blueprintItemResultContainer != null)
            {
                if (blueprint != null)
                {
                    foreach (Transform child in blueprintItemResultContainer) Destroy(child.gameObject);

                    foreach (var row in blueprint.resultItems)
                    {
                        var wrapper = Instantiate(blueprintItemResultPrefab);
        ...
        [Header("UI elements")]
        public Text blueprintTitle;

        public Text blueprintDescription;

        public CurrencyGroupUI blueprintCraftCost;

        public UIShowValue blueprintCraftingProgress = new();
//        public UnityEngine.UI.Slider blueprintCraftProgressSlider;

        [Required]
        public Button blueprintCraftButton;
