    ...
    public class InventoryUtility
    {
        ...
        public static void SetLayerRecursive(GameObject obj, int layer)
        {
            Assert.IsNotNull(obj,
                "Cannot set layers, gameObject given is null! (or object was destroyed while setting layers)");

            obj.layer = layer;
            foreach (Transform t in obj.transform) SetLayerRecursive(t.gameObject, layer);
        }
        ...
        public static void FindChildTransform(Transform startObject, string path, ref Transform result)
        {
            ...
            foreach (Transform child in startObject.transform)
            {
                ...
                if (child.transform.childCount > 0) FindChildTransform(child, path, ref result);
            }
        }
    }
}