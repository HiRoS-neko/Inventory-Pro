...
{
    [Serializable]
    public class CurrencyDecorator
    {
            ...
            if (_amount < 0f)
            {
                ...
                foreach (var lookup in parentGroup.lookups)
                    if (conversionDict.ContainsKey(lookup.currency))
                    {
                        ...
                        var amountInOtherCurrency = Mathf.Abs(_amount * conversionDict[lookup.currency]);
                ...
                {
                    var convertable =
                        lookup.currency.currencyConversions.FirstOrDefault(o =>
                            o.currency == currency && o.useInAutoConversion);
                    if (convertable != null)
                    {
                        var amountToGrab = Mathf.Abs(_amount) / convertable.factor;
        ...
        protected virtual void TryAutoConvertFractions()
        {
            ...
            if (currency.autoConvertFractions)
            {
                ...
                fraction = Math.Round(fraction,
                    4); // Because of so much conversion float point accuracy is a problem, so round it.
                if (fraction > 0.00001f)
                {
                    ...
                    var convertToConversionList =
                        currency.currencyConversions.FirstOrDefault(o =>
                            o.currency == currency.autoConvertFractionsToCurrency);
                    ...
                    convertTo.amount +=
                        (float)(fraction *
                                convertToConversionList
                                    .factor); // Works recursive, calls the next currency that can convert it down.
                }
            }
            else
            {
        ...
        protected virtual void TryAutoConvertCurrency()
        {
            ...
            if (currency.autoConvertOnMax)
                if (_amount >= currency.autoConvertOnMaxAmount)
                {
                    ...
                    var convertToConversionList =
                        currency.currencyConversions.FirstOrDefault(
                            o => o.currency == currency.autoConvertOnMaxCurrency);
                    if (convertToConversionList == null)
                    {
                        Debug.LogError("Can't convert to this type, conversion is not set in conversion list.");
                        return;
                    ...
                    var canExtractTimes = Mathf.FloorToInt(_amount / currency.autoConvertOnMaxAmount);
                    _amount -= currency.autoConvertOnMaxAmount * canExtractTimes;

                    convertTo.amount += canExtractTimes * currency.autoConvertOnMaxAmount *
                                        convertToConversionList.factor; // Works recursive
                }
        }
        ...
        public virtual float CanRemoveCount(bool allowCurrencyConversions)
        {
            ...
            {
                Debug.LogWarning(
                    "Can't convert currency because this lookup is not in a container. (can't grab other currency amounts)");
                return amount;
            }
            ...
            foreach (var tuple in dict)
            {
                ...
                totalAmount +=
                    lookup.amount /
                    tuple.Value; // How much the higher currency is worth in this currency ( gold worth in copper )
            }
        ...
        /// </summary>
        /// <returns></returns>
        private float _GetConversionFactor(Dictionary<CurrencyDefinition, float> dict, CurrencyDefinition fromCurrency,
            float factor)
        {
            foreach (var lookup in parentGroup.lookups)
            {
                // Find currency that converts to our current currency.
                var convertToCurrent =
                    lookup.currency.currencyConversions.FirstOrDefault(o =>
                        o.currency == fromCurrency && o.useInAutoConversion);
        ...
        /// </summary>
        /// <param name="amountMultiplier">
        ///     A multiplier to increase the amount. For example a boot costs 5 gold, I sold it 10 times
        ///     -> a multiplier of 10
        /// </param>
        /// <returns></returns>
