...
namespace Devdog.InventoryPro.UI
{
    public static class InventoryUIDragUtility
    {
        ...
        {
            var copy = Object.Instantiate(from);
            ...
            copyComp.transform.localPosition = new Vector3(copyComp.transform.localPosition.x,
                copyComp.transform.localPosition.y, 0.0f);
            copyComp.localScale = Vector3.one;
            copyComp.sizeDelta = from.GetComponent<RectTransform>().sizeDelta;

            var singleSize = copyComp.sizeDelta.x / copy.item.layoutSizeCols;
            var halfSize = singleSize / 2f;
            copyComp.pivot = new Vector2(halfSize / copyComp.sizeDelta.x,
                (copyComp.sizeDelta.y - halfSize) / copyComp.sizeDelta.y);
        ...
        public static UIDragModel OnBeginDrag(ItemCollectionSlotUIBase originalWrapper, PointerEventData eventData)
        {
            ...
            {
                Debug.LogWarning("Item still attached to cursor, can only drag one item at a time",
        ...
        /// </summary>
        public static bool isDraggingItem => draggingItem != null;


        public static List<DragHandlerBase> dragHandlers { get; }
