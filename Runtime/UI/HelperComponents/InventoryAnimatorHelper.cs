...
namespace Devdog.InventoryPro.UI
{
    ...
    public class InventoryAnimatorHelper : MonoBehaviour
    {
...
//            _startRotation = transform.localRotation;
        ...
        public void Play(string name)
        {
            ...
            if (result.Groups.Count == 3)
            {
                ...
                {
                    // Multiple params
                    var paramStrings = result.Groups[2].Value.Split(',');
                    ...
                    foreach (var param in paramStrings)
                    {
                        ...
                        float paramFloat;
                        var parsed = float.TryParse(p, out paramFloat);
                        if (parsed == false)
                        {
                            Debug.LogWarning(
                                "Parameter (" + p + ") passed is not a number, use (0.3), not (0.3f) or (0,3).",
                                transform);
                ...
                {
                    float waitTime;
                    var parsed = float.TryParse(result.Groups[2].Value, out waitTime);
                    if (parsed == false)
                    {
                        Debug.LogWarning("Parameter passed is not a number, use (0.3), not (0.3f) or (0,3).",
                            transform);
                        _animator.enabled = false;
                        return;
                    }
            ...
            {
                Debug.LogWarning("InventoryAnimatorHelper: Regex failed, most the string passed is likely faulty.",
                    transform);
            }
        }
