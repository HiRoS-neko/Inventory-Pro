...
namespace Devdog.InventoryPro
{
    ...
    public class ItemCollectionSlotUI : ItemCollectionSlotUIBase, IBeginDragHandler, IEndDragHandler, IDragHandler,
        IPointerUpHandler, IPointerDownHandler, IPointerEnterHandler, IPointerExitHandler
    {
        protected virtual void Awake()
        {
            if (icon != null) _startIcon = icon.sprite;
        }

        protected virtual void OnEnable()
        {
            if (useCustomUpdate) StartCustomUpdate();
        }

        protected virtual void OnDisable()
        {
            if (useCustomUpdate) StopCustomUpdate();

            if (currentlyHoveringSlot == this) currentlyHoveringSlot = null;

            if (pointerDownOnUIElement == this) pointerDownOnUIElement = null;
        ...
        protected virtual void CustomUpdate()
        {
            ...
            if (isPressingButton && pointerDownOnUIElement == this && InventoryUIDragUtility.isDraggingItem == false &&
                isLongPress)
                OnLongTap(new PointerEventData(EventSystem.current),
                    InventoryActionInput.EventType.All); // Long press for mobile
        }
        ...
        public override void Repaint()
        {
            ...
            {
                if (amountText != null) SetActive(amountText, false);

                if (itemName != null) SetActive(itemName, false);
        ...
        {
            // Check to avoid GC
            if (b.gameObject.activeSelf != set) b.gameObject.SetActive(set);
        }
...
//            cooldownVisualizer.Repaint(0.0f, 1.0f);
        ...
        public UIShowValue cooldownVisualizer = new();
        public bool disableIconWhenEmpty;
        ...
        private bool isPressingButton => pointerDownOnUIElement != null;

        private bool isLongPress => Mathf.Approximately(lastDownTime, 0f) == false &&
                                    Time.timeSinceLevelLoad -
                                    InventorySettingsManager.instance.settings.mobileLongPressTime > lastDownTime;

        private bool isDoubleTap => Mathf.Approximately(lastDownTime, 0f) == false &&
                                    Time.timeSinceLevelLoad < lastDownTime +
                                    InventorySettingsManager.instance.settings.mobileDoubleTapTime &&
                                    Mathf.Approximately(lastDownTime, Time.timeSinceLevelLoad) == false;
        ...
        public virtual void OnBeginDrag(PointerEventData eventData)
        {
            ...
            if (item != null && itemCollection.canDragInCollection) InventoryUIDragUtility.OnBeginDrag(this, eventData);
        }
        ...
        public virtual void OnEndDrag(PointerEventData eventData)
        {
            ...
            // for mobile...
            var endOnData = UIUtility.PointerOverUIObject(InventoryManager.instance.uiRoot,
                InventoryUIUtility.mouseOrTouchPosition);
            ...
            if (lookup.endOnSlot)
            {
                ...
                // Place on a slot
                if (InventorySettingsManager.instance.settings.useUnstackDrag &&
                    lookup.endItemCollection.useReferences == false &&
                    lookup.endItemCollection.canUnstackItemsInCollection)
                    if (InventorySettingsManager.instance.settings.unstackKeys.AllPressed(eventData,
                ...
                lookup.startItemCollection.SwapOrMerge((uint)lookup.startIndex, lookup.endItemCollection,
                    (uint)lookup.endIndex);
            }
        ...
        public virtual void OnPointerDown(PointerEventData eventData)
        {
            ...
            var tapped =
                OnTap(eventData, InventoryActionInput.EventType.OnPointerDown); // Mobile version of OnPointerUp
            if (tapped) return;
            ...
            if (s.useContextMenu &&
                s.triggerContextMenuKeys.AllPressed(eventData, InventoryActionInput.EventType.OnPointerDown))
                if (item != null)
                {
                    TriggerContextMenu();
                    return;
...
//            lastDownTime = 0f;
            ...
            var tapped = OnTap(eventData, InventoryActionInput.EventType.OnPointerUp); // Mobile version of OnPointerUp
            if (tapped) return;

            var s = InventorySettingsManager.instance.settings;
            if (s.useContextMenu &&
                s.triggerContextMenuKeys.AllPressed(eventData, InventoryActionInput.EventType.OnPointerUp))
                if (item != null)
                {
                    TriggerContextMenu();
                    return;
            ...
            if (item != null && InventoryUIDragUtility.isDraggingItem == false)
            {
                ...
                if (itemCollection.useReferences == false &&
                    InventorySettingsManager.instance.settings.useUnstackClick &&
                    itemCollection.canUnstackItemsInCollection)
                    if (InventorySettingsManager.instance.settings.unstackKeys.AllPressed(eventData,
                            InventoryActionInput.EventType.OnPointerUp))
                    {
                ...
                // Use the item
                if (InventorySettingsManager.instance.settings.useItemKeys.AllPressed(eventData,
                        InventoryActionInput.EventType.OnPointerUp))
                    TriggerUse();
            }
        }
        ...
        protected virtual bool CheckMobileInput(InventoryActionInput.MobileUIActions tap,
            InventoryActionInput.EventType eventUsed, PointerEventData eventData)
        {
            var s = InventorySettingsManager.instance.settings;
        ...
        public override void TriggerContextMenu()
        {
            ...
            foreach (var i in itemList) contextMenu.AddMenuOption(i.actionName, item, i.useItemCallback);
        ...
        public override void TriggerUnstack(ItemCollectionBase toCollection, int toIndex = -1)
        {
            ...
            if (item.currentStackSize > 1)
            {
                ...
                if (m.settings.useUnstackDialog)
                {
                    ...
                    InventoryManager.instance.unstackDialog.ShowDialog(itemCollection.transform, d.title, d.message, 1,
                        (int)item.currentStackSize - 1, item,
                        val =>
                        {
                            if (toIndex != -1)
                                itemCollection.UnstackSlot(index, toCollection, (uint)toIndex, (uint)val);
                            else
                                itemCollection.UnstackSlot(index, (uint)val);
                        },
                        val =>
                        {
                            // Canceled
                ...
                {
                    if (toIndex != -1)
                        itemCollection.UnstackSlot(index, toCollection, (uint)toIndex,
                            (uint)Mathf.Floor(item.currentStackSize / 2));
                    else
                        itemCollection.UnstackSlot(index, (uint)Mathf.Floor(item.currentStackSize / 2));
                }
            }
        }
        ...
        public override void TriggerDrop(bool useRaycast = true)
        {
            ...
            if (s.settings.showConfirmationDialogWhenDroppingItem &&
                InventoryManager.instance.confirmationDialog == null)
            {
                Debug.LogError(
                    "Trying to drop item with confimration dialog, but dialog is not assigned, please check your settings.");
                return;
            }
            ...
            if (s.settings.showConfirmationDialogWhenDroppingItem)
            {
                ...
                InventoryManager.instance.confirmationDialog.ShowDialog(itemCollection.transform, msg.title,
                    msg.message, item,
                    dialog => { tempItem.Drop(); },
                    dialog =>
                    {
