    ...
    public class BasicCollectionSorter : CollectionSorterBase
    {
        ...
        public override IList<InventoryItemBase> Sort(IList<InventoryItemBase> items)
        {
            ...
            var test = items.OrderByDescending(o => o.layoutSize).GroupBy(o => o.ID)
                .Select(o => new { item = o.First(), newStackSize = o.Sum(i => i.currentStackSize) })
                . // Get the items so we can re-assemble the items.
                ToList();
            ...
            for (var i = 0; i < test.Count; i++)
            {
                ...
                var currentStackSize = item.currentStackSize;
                while (currentStackSize > item.maxStackSize)
                {
                    ...
                    var a = Instantiate(item); // Copy the item
            ...
            test.ForEach(o => Destroy(o.item.gameObject));
            ...
            return sortedList.OrderBy(o => o.GetType().Name).ThenByDescending(o => o.layoutSize)
                .ThenBy(o => o.category.ID).ThenBy(o => o.name).ToArray(); // Order by and return
