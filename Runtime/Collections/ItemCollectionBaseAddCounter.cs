...
//        private Tuple[] _tuples = new Tuple[0];
        ...
        public List<CollectionPriority> collections { get; } = new();
        ...
        private void LoadCollectionTuples(CollectionPriority lookup)
        {
            ...
            for (var i = 0; i < lookup.collection.Length; i++) lookup.collection[i] = new Tuple(null, 0);
        ...
        public void SetItem(CollectionPriority lookup, uint i, Tuple t)
        {
            ...
            if (t.itemID != null)
            {
                Assert.IsTrue(t.itemID.Value < ItemManager.database.items.Length,
                    "Given item with ID " + t.itemID +
                    " does not exist in the current database! Make sure you're only adding items that are defined in the database.");
                var item = ItemManager.database.items[t.itemID.Value];
                if (item.layoutSize > 1)
                    for (var col = 0; col < item.layoutSizeCols; col++)
                    for (var row = 0; row < item.layoutSizeRows; row++)
                    {
                        if (row == 0 && col == 0)
                            continue;

                        var iTemp = (uint)(i + col + row * lookup.collectionRef.colsCount);
                        if (iTemp >= lookup.collection.Length) continue;
        ...
        {
            foreach (var lookup in collections)
        ...
        {
            return lookup.collection.Sum(o =>
                o.itemID == null ? 0.0f : ItemManager.database.items[o.itemID.Value].weight * o.amount);
        }
        ...
        public uint CanAddItemCount(CollectionPriority lookup, InventoryItemBase itemToAdd, uint earlyBailAmount)
        {
            ...
            {
                DevdogLogger.LogVerbose("Can't put item in this collection - " + lookup.collectionRef.collectionName,
                    lookup.collectionRef);
                return 0;
            }

            if (lookup.collectionRef.useReferences) return 0;
            ...
            var weightLimit = 99999;
            if (lookup.collectionRef.restrictByWeight && itemToAdd.weight > 0.0f) // avoid dividing by 0.0f
            {
                var weightSpace = lookup.collectionRef.restrictMaxWeight - GetWeight(lookup);
            ...
            for (uint i = 0; i < lookup.collection.Length; i++)
            {
                if (amount >= earlyBailAmount) break; // Got enough to pass.
        ...
        public void StartTest(CollectionPriority lookup)
        {
            ...
            for (var i = 0; i < testTuples.Length; i++)
        ...
        public bool CanSetItem(uint toSlot, InventoryItemBase item, CollectionPriority collection)
        {
            ...
            if (collection.collectionRef.ignoreItemLayoutSizes == false)
            {
                ...
                if (collection.collection[toSlot].blockedBy.HasValue) return false;
                ...
                for (var row = 0; row < item.layoutSizeRows; row++)
                {
                    ...
                    if (checkSlot >= collection.collectionRef.items.Length) return false; // Out of bounds
                    // break;
                    if (collection.collection[checkSlot].itemID.HasValue) return false;

                    if (collection.collection[checkSlot].blockedBy.HasValue) return false;
                }
            ...
            if (collection.collectionRef.canPutItemsInCollection == false) return false;

            if (collection.collectionRef.restrictByWeight &&
                collection.collectionRef.GetWeight() + item.weight * item.currentStackSize >
                collection.collectionRef.restrictMaxWeight)
            {
                DevdogLogger.LogVerbose("Can't add item to collection, adding item would exceed max weight",
            ...
            if (collection.collectionRef.VerifyFilters(item) == false)
            {
                DevdogLogger.LogVerbose(
                    "Can't add item to collection, restriction filter does not allow: #" + item.ID + " " +
                    item.GetType(), collection.collectionRef);
                return false;
            }
        ...
        public ItemAmountRow[] TryAdd(IList<ItemAmountRow> itemsToAdd)
        {
            ...
            itemsToAdd =
                itemsToAdd.OrderByDescending(o => o.item.layoutSize)
            ...
            for (var j = 0; j < itemsToAdd.Count; j++)
            {
                ...
                for (var i = 0; i < collection.collection.Length; i++)
                    if (collection.collection[i].itemID == itemsToAdd[j].item.ID)
                    {
                        ...
                        var canAddAmount = itemsToAdd[j].item.maxStackSize - collection.collection[i].amount;
        ...
        public void RemoveSlot(CollectionPriority lookup, uint slot)
        {
            ...
            foreach (var source in lookup.collection.Where(o => o.blockedBy == slot)) source.Reset();
        }
        ...
        {
            return collections.FirstOrDefault(o => o.collectionRef == collection);
        }

        public bool CanRemoveSlots(CollectionPriority lookup, uint amount)
        {
            if (lookup.collection.Length - (int)amount < 0) return false;

            var oldSize = (uint)lookup.collection.Length;
