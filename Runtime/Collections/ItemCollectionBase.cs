...
namespace Devdog.InventoryPro
{
    ...
    public partial class ItemCollectionBase : MonoBehaviour, IEnumerable<ICollectionItem>
    {
        ...
        /// <summary>
        ///     Occurs when an item inside this collection is used, some items reduce in stackSize when used ( Consumable for
        ///     example ).
        /// </summary>
        public event UsedItem OnUsedItem;

        /// <summary>
        ///     Occurs when a reference inside this collection is used, some items reduce in stackSize when used ( Consumable for
        ///     example ).
        /// </summary>
        ...
        /// <summary>
        ///     Occurs when an item is removed from a collection, either dropped, stored, sold, swapped, last item in stack used,
        ///     etc.
        ...
        ///     Note: The event is fired on both collections, if it is swapped within the same collection only that collection will
        ///     be called once.
        ...
        protected virtual void FillUI()
        {
            if (manuallyDefineCollection == false)
            {
                ...
                {
                    items[i] = CreateUIItem<ICollectionItem>(i,
                        itemButtonPrefab != null
                            ? itemButtonPrefab
                            : InventorySettingsManager.instance.settings.itemButtonPrefab);
                    items[i].Repaint();
                }
            }
            else
        ...
        {
            var item = Instantiate(prefab).GetComponent<T>();
        ...
        public static ItemCollectionBase FindByName(string collectionName)
        {
            ...
            resources = resources.Where(o => EditorUtility.IsPersistent(o) == false).ToArray();
        ...
        /// </summary>
        /// <param name="item">
        ///     A list of the item that was added, all slots where the item was stored. If the item doesn't fit 1
        ///     slot it's broken up into multiple slots.
        /// </param>
        ...
        /// <param name="droppedObj">
        ///     The actual 3D object that got dropped. This can either be the "DropObject" that has been set
        ///     in the cateogry or the actual model.
        /// </param>
        public delegate void DroppedItem(InventoryItemBase item, uint slot, GameObject droppedObj);

        /// <summary>
        ///     Called when an reference is removed from this collection. Note that it will only be called if "Use references" is
        ...
        /// <param name="itemID">
        ///     ItemID of the item that was removed from the inventory. ID instead of an actual item, because it
        ///     could have been destroyed in the process.
        /// </param>
        ...
        public delegate void UnstackedItem(ItemCollectionBase fromCollection, uint startSlot,
            ItemCollectionBase toCollection, uint endSlot, uint amount);
        ...
        public delegate void MergedSlots(ItemCollectionBase fromCollection, uint fromSlot,
            ItemCollectionBase toCollection, uint toSlot);
        ...
        public delegate void UsedReference(InventoryItemBase actualItem, uint itemID, uint referenceSlot,
            uint amountUsed);
        ...
        public delegate void SwappedItems(ItemCollectionBase fromCollection, uint fromSlot,
            ItemCollectionBase toCollection, uint toSlot);
        ...
        /// </summary>
        public uint collectionSize => (uint)_items.Length;
        ...
        /// <returns></returns>
        public ICollectionItem this[uint i] => items[i];

        public ICollectionItem this[int i] => items[i];
        ...
        /// </summary>
        public bool manuallyDefineCollection;
        ...
        public InventoryItemFilters filters = new();

        public ItemCollectionBaseAddCounter counter { get; } = new();
        ...
        {
            get => (uint)Mathf.Min(items.Length, _colsCount);
            protected set => _colsCount = value;
        }

        public uint rowsCount => (uint)Mathf.Max(1, Mathf.CeilToInt((float)items.Length / colsCount));
        ...
        public DynamicLayoutGroup layoutGroup
        {
            get
            {
                if (_layoutGroup == null)
                {
                    ...
                    if (_layoutGroup != null) colsCount = (uint)_layoutGroup.columnsCount;
                }
        ...
        public bool ignoreItemLayoutSizes;

        private bool _isDirty = true;

        public int disableCounterRebuildBlocks { get; set; }

        public bool disableCounterRebuild => disableCounterRebuildBlocks > 0;
        ...
        /// </summary>
        public List<CanAddItemDelegate> canAddItemToCollectionConditionals { get; protected set; } = new();
        ...
        public virtual void NotifyItemRemoved(InventoryItemBase item, uint itemID, uint slot, uint amount)
        {
            ...
            if (OnRemovedItem != null) OnRemovedItem(item, itemID, slot, amount);
        ...
        public virtual void NotifyItemAdded(IEnumerable<InventoryItemBase> item, uint amount, bool cameFromCollection)
        {
            ...
            if (OnAddedItem != null) OnAddedItem(item, amount, cameFromCollection);
        ...
        public void NotifyItemSwapped(ItemCollectionBase fromCollection, uint fromSlot, ItemCollectionBase toCollection,
            uint toSlot)
        {
            _isDirty = true;
        ...
        public void NotifyUnstackedItem(uint slot, ItemCollectionBase toCollection, uint toSlot, uint amount)
        {
            ...
            if (OnUnstackedItem != null) OnUnstackedItem(this, slot, toCollection, toSlot, amount);
        ...
        private void NotifyMergedSlots(ItemCollectionBase fromCollection, uint fromSlot,
            ItemCollectionBase toCollection, uint toSlot)
        {
            _isDirty = true;

            if (OnMergedSlots != null) OnMergedSlots(fromCollection, fromSlot, toCollection, toSlot);

            fromCollection.NotifyRequireLayoutRedraw();
            if (fromCollection != toCollection) toCollection.NotifyRequireLayoutRedraw();
        }
        ...
        public virtual bool AddSlots(uint amount, bool fireEvents = true)
        {
            ...
            for (uint i = 0; i < oldSize; i++) currentItems[i] = items[i];
            for (var i = oldSize; i < newSize; i++)
            {
                currentItems[i] = CreateUIItem<ICollectionItem>(i,
                    itemButtonPrefab != null
                        ? itemButtonPrefab
                        : InventorySettingsManager.instance.settings.itemButtonPrefab);
        ...
        public virtual bool CanRemoveSlots(uint amount)
        {
            ...
            return counter.CanRemoveSlots(counter.FindLookup(this), amount);
        }

        /// <summary>
        ///     Remove slots from the collection, slots are removed at the end, if however the slots are not empty the function
        ...
        public virtual bool RemoveSlots(uint amount, bool force = false, bool fireEvents = true)
        {
            ...
            var currentItems = new ICollectionItem[newSize];
            for (uint i = 0; i < newSize; i++) currentItems[i] = items[i];

            // Remove the old
            for (var i = newSize; i < oldSize; i++)
            {
                var c = items[i] as Component;
        ...
        {
            RebuildCounterIfDirty();
            return counter.CanAddItemCount(counter.FindLookup(this), itemToAdd, earlyBailAmount);
        }
        ...
        {
            RebuildCounterIfDirty();
            return counter.CanAddItem(counter.FindLookup(this), item);
        }
        ...
        public bool CanAddItems(InventoryItemBase[] items)
        {
            ...
            var unAddedItems = counter.TryAdd(items);
        ...
        public bool CanAddItems(ItemAmountRow[] items)
        {
            ...
            var unAdded = counter.TryAdd(items);
        ...
        {
            RebuildCounterIfDirty();
            if (CanAddItems(items) == false) return false;
            ...
            {
                var added = AddItem(item, storedItems, repaint, fireEvents);
                Assert.IsTrue(added,
                    "Couldn't add items even though check passed! Please report this bug + stack trace");
            }
        ...
        ///     Should events be fired such as OnAddedItem? Leave to true unless you know what you're doing...
        ///     Don't say I didn't warn you...
        /// </param>
        /// <returns>Returns true if the item was placed, false if not.</returns>
        public virtual bool AddItem(InventoryItemBase item, ICollection<InventoryItemBase> storedItems = null,
            bool repaint = true, bool fireEvents = true)
        {
            ...
            using (new CollectionAvoidRebuildLock(this))
            {
                ...
                if (currency != null) return currency.PickupItem();

                if (canPutItemsInCollection == false)
                {
                    DevdogLogger.LogVerbose(
                        "Tried to put item in collection, but setting - canPutItemsInCollection is false.", gameObject);
                    return false;
                }

                // Instantiate the object if it's not an instance object (A prefab for example...)
                if (item.IsInstanceObject() == false)
                    item = Instantiate(item); // Create an instance object
                ...
                uint placedCount = 0;
                var cameFromCollection = item.itemCollection != null;
                    ...
                    for (var i = 0; i < items.Length; i++)
                    {
                        ...
                        if (items[i].item.Equals(item, false))
                        {
                            ...
                            if (item.currentStackSize > item.maxStackSize)
                            {
                                ...
                                var added = item.maxStackSize - items[i].item.currentStackSize;
                                placedCount += added;
                ...
                else
                {
                    ...
                    {
                        InventoryManager.langDatabase.collectionExceedingMaxWeight.Show(item.name, item.description,
                            collectionName);
                        return false;
                    }
        ...
        public virtual bool AddItemAndRemove(InventoryItemBase item, ICollection<InventoryItemBase> storedItems = null,
            bool repaint = true)
        {
            var oldCollection = item.itemCollection;
            var oldIndex = item.index;

            var added = AddItem(item, storedItems, repaint);
            if (added)
            {
        ...
        public bool RemoveItemsThenAdd(InventoryItemBase[] itemsToAdd, params ItemAmountRow[] itemsToRemoveFirst)
        {
            ...
            counter.TryRemoveItems(itemsToRemoveFirst);
            var unAdded = counter.TryAdd(itemsToAdd);
            if (unAdded.Length > 0) return false;

            foreach (var item in itemsToRemoveFirst)
            {
                var removed = RemoveItem(item.item.ID, item.amount);
                Assert.IsTrue(removed == item.amount);
            }

            var added = AddItems(itemsToAdd);
            Assert.IsTrue(added);
        ...
        /// <returns>
        ///     The amount of item that were removed, note that when there are 4 items in this collection and you try to
        ///     remove 10, 4 will be removed and the value 4 (amount of items removed) is returned.
        /// </returns>
...
//                    item.itemCollection[item.index].item = null;
                ...
                else if (removed < amountToRemove)
                {
                    ...
                    var toRemove = amountToRemove - removed;
        ...
        {
            Assert.IsTrue(item.itemCollection == this,
                "Trying to remove specific item from collection, but the item isn't in this collection.");
...
//            items[item.index].item = null;
        ...
        public float CanAddCurrencyCount(uint currencyID)
        {
            ...
            if (currency != null) total += currency.CanAddCount();
        ...
        /// <param name="allowCurrencyConversions">
        ///     When true all possible conversions will be considered. For example 1 gold is
        ///     worth 100 silver. If you try to remove 101 silver, gold will be converted down to silver.
        /// </param>
        /// <returns></returns>
        ...
        {
            RebuildCounterIfDirty();
            return counter.FindFirstEmptySlotForItem(counter.FindLookup(this), item);
        }
        ...
        public virtual bool UnstackSlot(uint slot, uint amount, bool repaint = true)
        {
            ...
            var firstEmptySlot = FindFirstEmptySlot(items[slot].item);
            if (firstEmptySlot != -1) return UnstackSlot(slot, this, (uint)firstEmptySlot, amount, repaint);
        ...
        /// <param name="repaint"></param>
        /// <returns>Returns true if unstacked, false if failed</returns>
        public virtual bool UnstackSlot(uint slot, ItemCollectionBase toCollection, uint toSlot, uint amount,
            bool repaint = true)
        {
            // References can never be unstacked / stacked.
            if (useReferences || canUnstackItemsInCollection == false)
            ...
            if (toCollection[toSlot].item != null)
            {
                ...
                {
                    DevdogLogger.LogVerbose(
                        "Trying to unstack into a filled slot, but stack amount would override max stack size.");
                    return false;
                }
            }
            ...
            else
            {
                ...
                var copy = Instantiate(items[slot].item);
        ...
        /// </summary>
        protected virtual bool SwapSlots(uint fromSlot, ItemCollectionBase toCollection, uint toSlot,
            bool repaint = true, bool fireEvents = true)
        {
            RebuildCounterIfDirty();
            using (new CollectionAvoidRebuildLock(this))
            {
                var fromCollection = this;
...
//                var toItem = toCollection[toSlot].item;
                ...
                var fromTempAmount = fromCollection[fromSlot].item != null
                    ? fromCollection[fromSlot].item.currentStackSize
                    : 0; // TODO: fromCollection item shouldn't be empty!
                //uint toTempAmount = toCollection[toSlot].item.currentStackSize;

                // Move to first item to toCollection
                var moved = fromCollection.MoveItem(fromCollection[fromSlot].item, fromSlot, toCollection, toSlot, true,
                    false); // Moves the item into toCollection[toSlot]
                ...
                {
                    var moved2 = toCollection.MoveItem(tempTo, toSlot, fromCollection, fromSlot, false, false);
                    if (moved2 == false) DevdogLogger.LogWarning("Couldn't move 2nd object.");
                }
                ...
                if (fromCollection != toCollection)
                    ...
                    {
                        if (toCollection[toSlot].item != null)
                            fromCollection.NotifyItemRemoved(toCollection[toSlot].item, toCollection[toSlot].item.ID,
                                fromSlot, toCollection[toSlot].item.currentStackSize);

                        // if temp == null then the slot we're moving to didn't have an object before, so then one won't be removed.
                        if (tempTo != null)
                            fromCollection.NotifyItemAdded(fromCollection[fromSlot].item,
                                fromCollection[fromSlot].item.currentStackSize, true);
                    }
        ...
        public virtual bool CanMergeSlots(uint slot1, ItemCollectionBase collection2, uint slot2)
        {
            ...
            if (collection1.items[slot1].item.Equals(collection2.items[slot2].item, false))
                return collection1.items[slot1].item.currentStackSize +
                       collection2.items[slot2].item.currentStackSize <=
                       collection1.items[slot1].item.maxStackSize; // Does it fit?
            ...
            bool fireEvents = true)
        {
            ...
            {
                var tempStackSize = collection1.items[slot1].item.currentStackSize;
...
//                collection1[slot1].item = null;
                ...
                {
                    collection2.NotifyItemAdded(collection2[slot2].item, tempStackSize,
                        true); // Items are moved to collection2 so it gains the stack of the other
                    collection1.NotifyItemRemoved(tempItemCollection1, tempItemCollection1.ID, slot1,
                        tempStackSize); // Item in collection1 is moved to collection2, so it loses an object.

                    DevdogLogger.LogVerbose("Merged an item in another collection " + collection2 + " gained " +
                                            tempStackSize + " items of ID: " + collection2[slot2].item.ID +
                                            " and removed " + tempStackSize + " of " + collection1 + " with item ID: " +
                                            collection2[slot2].item.ID +
                                            "\nThere are now " + collection2.GetItemCount(collection2[slot2].item.ID) +
                                            " items in " + collection2 + " and " +
                                            collection1.GetItemCount(collection2[slot2].item.ID) + " in " +
                                            collection1);
                }
        ...
        {
            RebuildCounterIfDirty();
            return counter.CanSetItem(slot, item, counter.FindLookup(this));
        }
        ...
        public virtual void SetItems(InventoryItemBase[] toSet, bool setParent, bool repaint = true)
        {
            ...
            {
                if (items[i].item != null)
                    NotifyItemRemoved(items[i].item, items[i].item.ID, items[i].item.index,
                        items[i].item.currentStackSize);

                SetItem(i, toSet[i], repaint);
                if (items[i].item != null)
                {
                    var cameFromCollection = items[i].itemCollection != null;
                    items[i].itemCollection = this;
                    ...
                    if (setParent)
                    {
                        if (moveObjectsToChildren) items[i].item.transform.SetParent(containerItemsParent);
                        items[i].item.gameObject.SetActive(false);
                    }
                }
            }

            for (var i = toSet.Length; i < items.Length; i++)
            {
                if (items[i].item != null)
                    NotifyItemRemoved(items[i].item, items[i].item.ID, items[i].item.index,
                        items[i].item.currentStackSize);
        ...
        public virtual void SortCollection()
        {
            ...
            {
                var added = AddItem(item, null, false, false);
            ...
            NotifySorted();
            DevdogLogger.LogVerbose("Sorting collection: " + ToString());
        }

        /// <summary>
        ///     Places an object in a slot and handles repaint, this method can easilly be overwritten for extra functinoality when
        ///     placing an item.
        /// </summary>
        /// <param name="fromSlot"></param>
        /// <param name="toCollection"></param>
        ...
        public virtual bool MoveItem(InventoryItemBase item, uint fromSlot, ItemCollectionBase toCollection,
            uint toSlot, bool clearOld, bool doRepaint = true)
        {
            ...
            if (useReferences && toCollection.useReferences == false) return SetItem(fromSlot, null, doRepaint);

            // Copy to reference, without clearing it.
            if (useReferences == false && toCollection.useReferences)
            {
                var set = toCollection.SetItem(toSlot, item, doRepaint);
                if (set == false)
                    return false;
            ...
            var set2 = toCollection.SetItem(toSlot, item, doRepaint);
            if (set2 == false) return false;

            if (clearOld) return SetItem(fromSlot, null, doRepaint);
...
//                    items[i].item = null;
                    ...
                    if (fireEvents) NotifyItemRemoved(item, item.ID, i, item.currentStackSize);
                    items[i].Repaint();
                }
            }
        ...
        {
            RebuildCounterIfDirty();
            return counter.TryRemoveItems(items);
        }
        ...
        {
            RebuildCounterIfDirty();
            return counter.GetEmptySlotCount();
        }
